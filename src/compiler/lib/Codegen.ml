...
let rec gen_code_expr code_queue fxn_tbl jeroo_tbl meta_expr pane_num =
  ...
  | AST.BinOpExpr (e1, AST.And, e2) ->
    gen_code_expr code_queue fxn_tbl jeroo_tbl e1 pane_num;
    gen_code_expr code_queue fxn_tbl jeroo_tbl e2 pane_num;
  ...
  | AST.UnOpExpr (AST.Not, e) ->
    gen_code_expr code_queue fxn_tbl jeroo_tbl e pane_num;
  ...
  | AST.BinOpExpr ({ a = AST.IdExpr(id); _ }, AST.Dot, e) ->
    ...
    gen_code_expr code_queue fxn_tbl jeroo_tbl e pane_num
  | AST.FxnAppExpr ({ a = AST.IdExpr(id); _ }, args) -> gen_code_fxn code_queue fxn_tbl jeroo_tbl id args line_num pane_num
  | _ ->
...
and gen_code_fxn code_queue fxn_tbl _jeroo_tbl id args line_num pane_num =
  begin match id with
...
let gen_code_decl _code_queue _fxn_tbl jeroo_tbl id args line_num =
  let id_loc = Hashtbl.find jeroo_tbl id in
  match args with
  | [] ->
...
let rec gen_code_stmt code_queue fxn_tbl jeroo_tbl stmt pane_num =
  match stmt with
  | AST.BlockStmt stmts -> gen_code_block_stmt code_queue fxn_tbl jeroo_tbl stmts pane_num
  | AST.ExprStmt expr -> gen_code_expr_stmt code_queue fxn_tbl jeroo_tbl expr pane_num
      ...
      begin match expr with
        | AST.UnOpExpr (AST.New, { a = AST.FxnAppExpr ({ a = AST.IdExpr(ctor); _ }, args); _ }) ->
          if not (String.equal ctor "Jeroo") then
            ...
            Queue.add (gen_code_decl code_queue fxn_tbl jeroo_tbl id args line_num) code_queue;
            line_num
          end
        | _ -> raise (SemanticException {
            lnum = line_num;
  ...
  | AST.IfStmt(e, stmt, line_num) -> gen_code_if code_queue fxn_tbl jeroo_tbl e stmt line_num pane_num
  | AST.IfElseStmt(e, s1, s2, line_num) -> gen_code_if_else code_queue fxn_tbl jeroo_tbl e s1 s2 line_num pane_num
  | AST.WhileStmt(e, s, line_num) -> gen_code_while code_queue fxn_tbl jeroo_tbl e s line_num pane_num

and gen_code_block_stmt code_queue fxn_tbl jeroo_tbl stmts pane_num =
  (* fold_left will always return the line number of the last statement executed *)
  stmts
  |> List.fold_left (fun _ stmt -> gen_code_stmt code_queue fxn_tbl jeroo_tbl stmt pane_num) 0

and gen_code_expr_stmt code_queue fxn_tbl jeroo_tbl expr pane_num =
  match expr with
  | { a = (Some e); lnum = lnum } ->
    gen_code_expr code_queue fxn_tbl jeroo_tbl e pane_num;
    lnum
...
and gen_code_if code_queue fxn_tbl jeroo_tbl e stmt line_num pane_num =
  gen_code_expr code_queue fxn_tbl jeroo_tbl e pane_num;
  ...
  let end_lnum = gen_code_stmt code_queue fxn_tbl jeroo_tbl stmt pane_num in
  Queue.add (Bytecode.LABEL (jmp_lbl, pane_num, end_lnum)) code_queue;
  end_lnum

and gen_code_if_else code_queue fxn_tbl jeroo_tbl e s1 s2 line_num pane_num =
  ...
  let true_end_lnum = gen_code_stmt code_queue fxn_tbl jeroo_tbl s1 pane_num in
  ...
  let false_end_lnum = gen_code_stmt code_queue fxn_tbl jeroo_tbl s2 pane_num in
  Queue.add (Bytecode.LABEL (done_lbl, pane_num, false_end_lnum)) code_queue;
  false_end_lnum

and gen_code_while code_queue fxn_tbl jeroo_tbl e s line_num pane_num =
  ...
  let end_while_lnum = gen_code_stmt code_queue fxn_tbl jeroo_tbl s pane_num in
...
let gen_code_fxn code_queue fxn_tbl jeroo_tbl fxn pane_num =
  Hashtbl.add fxn_tbl fxn.id fxn.id;
  Queue.add (Bytecode.LABEL (fxn.id, pane_num, fxn.start_lnum)) code_queue;
  fxn.stmts
  |> List.iter (fun stmt -> let _ = gen_code_stmt code_queue fxn_tbl jeroo_tbl stmt pane_num in ());
  Queue.add (Bytecode.RETR (pane_num, fxn.end_lnum)) code_queue

let gen_code_main_fxn code_queue fxn_tbl jeroo_tbl fxn =
  if not (String.equal fxn.id "main") then
    ...
    gen_code_fxn code_queue fxn_tbl jeroo_tbl fxn 0
  end
...
let codegen translation_unit =
  ...
  |> List.iter (fun fxn -> gen_code_fxn code_queue fxn_tbl jeroo_tbl fxn 1);
  (* generate the code for the main function *)
  gen_code_main_fxn code_queue fxn_tbl jeroo_tbl translation_unit.main_fxn;
